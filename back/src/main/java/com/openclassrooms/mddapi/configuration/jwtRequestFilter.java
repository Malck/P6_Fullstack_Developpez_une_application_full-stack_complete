        ...
        if (header != null && header.startsWith("Bearer ")) {
            ...
            try {
                ...
                if(
                        ...
                        SecurityContextHolder.getContext().getAuthentication() == null) {  // Si aucun utilisateur n'est actuellement authentifié, cette méthode renverra null.

                    // Chargement des détails de l'utilisateur
                    CustomUserDetails userDetails = this.jwtUserDetailsService.loadUserByLogin(userLoginFromToken);

                    // On vérifie que le token n'est pas expiré et que son subject correspond à l'email de l'utilisateur
                    if(jwtTokenUtil.validateToken(token, userDetails, userLoginFromToken)){
                        ...
                        // l'utilisateur actuel est considéré comme authentifié
